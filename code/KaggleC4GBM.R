...
for (ind in ind_train) {
  ...
  casual <- gbm.fit(dataset[dataset$typ == 0, -11], y = log(y_casual + 1), distribution ="gaussian", shrinkage=0.1, 
                    n.trees = 100, interaction.depth = 6, verbose = F)
  registered <- gbm.fit(dataset[dataset$typ == 0, -11], y = log(y_reg + 1), distribution ="gaussian", shrinkage=0.1, 
                        n.trees = 100, interaction.depth = 6, verbose = F)
  pred_casual <- predict(casual, dataset[dataset$typ == 1, -11], n.trees = 100)
  pred_reg <- predict(registered, dataset[dataset$typ == 1, -11], n.trees = 100)

  res2 <- rbind(res2, as.matrix(round(exp(pred_casual) + exp(pred_reg) - 2, 0)))
  
}

sub <- read.csv("../sampleSubmission.csv")
sub$count <- res2
write.csv(sub, file = "../sub5.csv", row.names = F, quote = F)